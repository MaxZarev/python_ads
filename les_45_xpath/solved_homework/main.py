...
def main():
    driver = start_profile(949)

    for token_number in range(1, 11):  # перебираем номера токенов
        driver.get("https://pancakeswap.finance/info/v3/tokens") # открываем страницу с токенами для каждого токена
        time.sleep(3)
        xpath_anchor = f"//div[text()='{token_number}']"  # начало xpath для поиска токена с якорем от номера токена

        token_name = driver.find_element(By.XPATH, xpath_anchor + "/following-sibling::div[1]").text  # получаем имя токена
        token_name = token_name.replace("\n", " ")  # убираем перенос строки

        token_price = driver.find_element(By.XPATH, xpath_anchor + "/following-sibling::div[2]").text  # получаем цену токена
        token_volume = driver.find_element(By.XPATH, xpath_anchor + "/following-sibling::div[3]").text  # получаем объем токена
        token_tvl = driver.find_element(By.XPATH, xpath_anchor + "/following-sibling::div[4]").text  # получаем TVL токена

        driver.find_element(By.XPATH, xpath_anchor).click()  # кликаем на токен
        time.sleep(3)
        token_transactions = driver.find_element(By.XPATH, "//div[text()='Transactions 24H']/following-sibling::div").text  # получаем количество транзакций

        print(f"Token {token_name} - {token_price} - {token_volume} - {token_tvl} - {token_transactions}")
