...
from utils import sleep_random, generate_password, salt_password, shuffle_password, shuffle_seed, unshuffle_seed
...
class Metamask:
    ...
    def create_wallet(self):
        ...
        salted_password = salt_password(shuffle_password(password))
        self.ads.input_text("//input[@data-testid='create-password-new']", salted_password)
        sleep_random()
        self.ads.input_text("//input[@data-testid='create-password-confirm']", salted_password)
        sleep_random()
        ...
        seed_str = " ".join(seed)
        seed_str = shuffle_seed(seed_str)

        self.ads.excel.add_row([address, password, self.ads.profile_number, seed_str])
    ...
    def auth_metamask(self) -> None:
        ...
        if self.ads.find_element("//li[@data-testid='home__nfts-tab']", 3):
            return
        # вводим пароль
        salted_password = salt_password(shuffle_password(self.password))
        self.ads.input_text("//input[@data-testid='unlock-password']", salted_password)
    ...
    def import_wallet(self):
        ...
        """
        self.open_metamask()
        unshuffled_seed = unshuffle_seed(self.seed)

        seed_list = unshuffled_seed.split(" ")
        ...
        self.ads.excel.add_row([address, self.password, self.ads.profile_number, self.seed])
