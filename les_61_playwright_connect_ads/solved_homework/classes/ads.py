...
class Ads:
    ...
    def __init__(self, profile_number: int, proxy: str = ""):
        ...
        if set_proxy:
            self._set_proxy()
        self.context = self._start_profile()
        self.page = self.context.pages[0]
        if check_proxy:
            self._check_proxy()
        self.excel = Excel("wallets.xlsx")
        self.OKX = OKX(self)
        self.address = self.excel.get_cell(str(profile_number), "Address")


    def _start_profile(self) -> BrowserContext:
        """
        Запускает профиль адс и подключается к нему при помощи playwright,
        сохраняя контекст в self.
        :return: BrowserContext из профиля адс
        """
